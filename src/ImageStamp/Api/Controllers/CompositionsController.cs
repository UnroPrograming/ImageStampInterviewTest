    ...
    ILogger<CompositionsController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Create(CancellationToken cancellationToken)
    {
        ...
        string? uploadError = UploadValidation.ValidateUpload(baseImageFile, _options);
        if (uploadError is not null)
        {
            return Problem(uploadError, statusCode: StatusCodes.Status400BadRequest);
        }

        (List<Layer> layers, string? layerError) = await LayerMapper.ParseLayersAsync(
            form["layers"].ToString(), form, _options, cancellationToken);

        if (layerError is not null)
        {
            return Problem(layerError, statusCode: StatusCodes.Status400BadRequest);
        }
        ...
        composition.BaseImageFileName = compositionRequest.BaseImage.FileName;
        ...
        await SaveCompositionAsync(composition, result, compositionRequest.Layers, cancellationToken);
    ...
    public async Task<IActionResult> Batch(CancellationToken cancellationToken)
    {
        ...
        // Valida y materializa cada imagen base. El modelo de dominio (BaseImageInput)
        ...
        // Parsea las capas, compartidas por todas las imágenes base
        (List<Layer> layers, string? layerError) = await LayerMapper.ParseLayersAsync(
            form["layers"].ToString(), form, _options, cancellationToken);
        ...
        try
        {
            ...
            {
                CompositionRequest compositionRequest = new CompositionRequest();
                compositionRequest.BaseImage = baseImageInput;

                // Clona las capas para ESTA composición. Layer.Clone() se encarga de dar un
                // MemoryStream fresco a las capas de tipo imagen (los streams no son
                // reutilizables entre lecturas); blur y solid no tienen stream que clonar.
                List<Layer> layersForThisComposition = new List<Layer>();
                foreach (Layer layer in layers)
                {
                    layersForThisComposition.Add(layer.Clone());
                }
